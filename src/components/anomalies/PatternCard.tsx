...
export const PatternCard: React.FC<PatternCardProps> = ({ insight }) => {
  const getCategoryConfig = () => {
    switch (insight.category) {
      ...
      case 'wear':
        return {
          icon: Zap,
          badgeBg: 'bg-sky-500/15 text-sky-500 border-sky-500/40',
        };
      case 'behavioral':
      default:
        return {
          icon: UserCheck,
          badgeBg: 'bg-purple-500/15 text-purple-500 border-purple-500/40',
        };
    }
  };
  ...
  return (
    <div className="cab-panel p-4 border border-cat-border rounded-lg space-y-3 flex flex-col justify-between hover:border-cat-yellow/60 transition-colors shadow-sm">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-[10px] font-mono text-cat-muted flex items-center gap-1">
            ...
            AI Metric ({insight.confidence}%)
          </span>
        </div>

        {/* Reusable TrendIndicator */}
        <TrendIndicator trend={insight.trend} size="sm" />
      </div>
